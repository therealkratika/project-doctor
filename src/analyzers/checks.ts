...
export function runProjectChecks(): ProjectCheck[] {
...
checks.push({
  ...
  const isTypeScriptProject =
    hasTsConfig || hasTypeScriptDependency;
  ...
  const existingEnvFiles = envFiles.filter(
    (file) =>
      fs.existsSync(
        path.join(projectPath, file),
      ),
  ...
  if (existingEnvFiles.length > 0) {
    ...
    if (hasGitignore) {
      try {
        ...
        envIgnored = existingEnvFiles.every(
          (envFile) =>
