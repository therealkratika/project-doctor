export function analyzeTechnology(dependencies, devDependencies) {
    ...
    if (has(allDependencies, "mongoose") ||
        has(allDependencies, "mongodb")) {
        database = "MongoDB";
    }
    else if (has(allDependencies, "pg") ||
        has(allDependencies, "postgres")) {
        database = "PostgreSQL";
    }
    else if (has(allDependencies, "mysql") ||
        has(allDependencies, "mysql2")) {
        database = "MySQL";
    }
    else if (has(allDependencies, "mariadb")) {
        database = "MariaDB";
    }
    else if (has(allDependencies, "sqlite3") ||
        has(allDependencies, "better-sqlite3")) {
        database = "SQLite";
    }
    else if (has(allDependencies, "redis") ||
        has(allDependencies, "ioredis")) {
        database = "Redis";
    }
    else if (has(allDependencies, "cassandra-driver")) {
        database = "Cassandra";
    }
    else if (has(allDependencies, "oracledb")) {
        database = "Oracle";
    }
    else if (has(allDependencies, "mssql")) {
    ...
    // --------------------------------
    let orm = "None";
    if (has(allDependencies, "prisma") ||
        has(allDependencies, "@prisma/client")) {
        orm = "Prisma";
    }
    else if (has(allDependencies, "drizzle-orm")) {
        orm = "Drizzle";
    }
    else if (has(allDependencies, "typeorm")) {
        orm = "TypeORM";
    }
    else if (has(allDependencies, "sequelize")) {
        orm = "Sequelize";
    }
    else if (has(allDependencies, "mongoose")) {
        orm = "Mongoose";
    }
    else if (has(allDependencies, "knex")) {
        orm = "Knex";
    }
    else if (has(allDependencies, "objection")) {
        orm = "Objection.js";
    }
    else if (has(allDependencies, "mikro-orm")) {
        orm = "MikroORM";
    }
    return {
        framework,
...
// --------------------------------
function has(dependencies, dependency) {
